...
checkpoint = torch.load('models/model_state_niidgauss')
net = VDN(C, dep_U=dep_U, wf=64)
if use_gpu:
    net = torch.nn.DataParallel(net).cuda()
    net.load_state_dict(checkpoint)
else:
    state_dict_1 = net.state_dict()
    for key, value in state_dict_1.items():
        if "model." + key in checkpoint.keys():
            state_dict_1[key] = checkpoint["model." + key]
...
im_path = "G:\\LiangHao\\Master\Works\\work_1\\before_MyWork\\data\\train\\BSDS200\\15088.png"
...
print('PSNR={:5.2f}, SSIM={:7.4f}, time={:.4f}'.format( psnr_val,
                                                                         ssim_val, toc-tic))
